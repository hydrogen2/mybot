...
prolog.assertz("f_expr(f00)")
prolog.assertz("f_entity(f00, e0)")
prolog.assertz("f_value(f00, '(5*a + 10*b)')")
prolog.assertz("f_question(q1)")
prolog.assertz("f_frame(q1, f00)")
prolog.assertz("f_frameElement(q1, f_value)")

prolog.assertz("f_choice(c1)")
prolog.assertz("f_question(c1, q1)")
prolog.assertz("f_id(c1, 'A')")
prolog.assertz("f_value(c1, '18')")
...
prolog.assertz("f_expr(f260)")
prolog.assertz("f_entity(f260, e24)")
prolog.assertz("f_value(f260, '(x-1)')")

# prolog.assertz("f_expr_value(f261)")
# prolog.assertz("f_entity(f261, e24)")

prolog.assertz("f_question(q2)")
prolog.assertz("f_frame(q2, f260)")
prolog.assertz("f_frameElement(q2, f_value)")
print(list(prolog.query("solveQuant(f_expr, e21)")))
print(list(prolog.query("answer(q2, X)")))
